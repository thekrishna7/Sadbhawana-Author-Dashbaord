...
import { Home, BookOpen, Users, Bell, User, Loader2, LogOut } from "lucide-react";
...
export function DashboardShell({
  ...
  useEffect(() => {
    ...
    supabase.auth.getUser().then(async ({ data }) => {
      ...
      if (!p || (p.role !== "super_admin" && p.role !== "staff")) {
        router.replace("/login");
        return;
      }

      setAdminProfile(p as Profile);
      setLoading(false);
      ...
      channel = supabase
        .channel(`admin-shell-notifs-${data.user.id}`)
        .on(
          "postgres_changes",
          {
            event: "*",
          ...
          <div className="flex items-center gap-3">
            {adminProfile?.avatar_url ? (
              <div className="h-8 w-8 rounded-full overflow-hidden border border-white/10 relative">
                <Image src={adminProfile.avatar_url} alt={adminProfile.full_name} fill className="object-cover" />
              </div>
            ) : (
