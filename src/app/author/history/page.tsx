...
export default function AuthorFileHistoryPage() {
  ...
  const handleApproveFile = async () => {
    ...
    try {
      ...
      if (res.ok) {
        showToast('File approved successfully! Admin notified.', 'success');
        loadData();
      } else {
  ...
  const receivedCount = files.filter((f) => f.uploader?.role === 'ADMIN').length;
  const sentCount = files.filter((f) => f.uploader?.role === 'AUTHOR').length;
    ...
    <div className="space-y-6 animate-fade-in">
      <div>
        <h1 className="text-2xl font-extrabold text-slate-900 dark:text-slate-100 flex items-center gap-2.5">
          <History className="w-7 h-7 text-blue-600" /> My File History & Archives
        </h1>
        <p className="text-sm text-slate-500">Complete archive of files received from Admin, sent files, and manuscript version iterations.</p>
      </div>
      ...
      <div className="p-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 flex flex-col md:flex-row gap-3 items-center justify-between shadow-sm">
        <div className="relative w-full md:w-80">
          ...
          <input
            type="text"
            placeholder="Search by file name or book..."
            value={search}
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {filteredFiles.map((file) => {
            ...
            const isReceived = file.uploader?.role === 'ADMIN';
              ...
              >
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    ...
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-2 mb-1">
                        <span className={`px-2.5 py-0.5 rounded-md text-[10px] font-extrabold uppercase ${
                          ...
                          {isReceived ? 'RECEIVED FROM ADMIN' : 'SENT TO ADMIN'}
                        </span>
                      ...
                      <div className="flex flex-wrap items-center gap-4 text-xs text-slate-500 mt-2">
                        <span>Version: <strong className="text-blue-600">v{file.version}</strong></span>
                  ...
                  <div className="flex flex-wrap items-center gap-2 self-start md:self-center shrink-0">
                    ...
                    {isReceived && file.status !== 'APPROVED' && (
                      <button
