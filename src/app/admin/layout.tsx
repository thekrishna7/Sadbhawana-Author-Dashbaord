...
let cachedAdminUnread: number | null = null;
...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  const [currentUser, setCurrentUser] = useState<any>(cachedAdminUser);
  const [unreadCount, setUnreadCount] = useState<number>(cachedAdminUnread || 0);
      ...
      fetch('/api/notifications')
        ...
        .then((data: any) => {
          if (data?.unreadCount !== undefined) {
            cachedAdminUnread = data.unreadCount;
  ...
  const navItems = [
    ...
    { name: 'Uploaded Files', href: '/admin/uploads', icon: FolderUp },
    { name: 'File History', href: '/admin/history', icon: History },
      ...
      <aside className="hidden lg:flex flex-col w-64 border-r border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shrink-0 sticky top-0 h-screen z-30">
        ...
        <div className="p-4 border-b border-slate-200 dark:border-slate-800 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] font-bold tracking-widest text-blue-600 dark:text-blue-400 uppercase">ADMIN PORTAL</p>
          </div>
        </div>
        ...
        {/* Author Portal Quick Switch */}
        <div className="p-4 border-t border-slate-200 dark:border-slate-800">
          <Link
            href="/author"
            className="flex items-center justify-between p-3 rounded-xl bg-slate-100 dark:bg-slate-800/60 hover:bg-slate-200 dark:hover:bg-slate-800 text-xs font-semibold text-slate-700 dark:text-slate-300 transition-colors"
          >
        ...
        <div className="p-4 border-t border-slate-200 dark:border-slate-800 flex items-center justify-between">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <p className="text-xs font-bold text-slate-900 dark:text-slate-100 truncate">{currentUser?.fullName || 'Chief Admin'}</p>
        ...
        <header className="sticky top-0 z-20 border-b border-slate-200 dark:border-slate-800 glass-header px-4 md:px-8 py-3.5 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div className="hidden sm:flex items-center gap-2 text-xs font-semibold text-slate-500">
              ...
              <span>Admin Authority Control Desk</span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            >
              ...
              />
              <span className="hidden sm:inline text-slate-700 dark:text-slate-300">{currentUser?.fullName?.split(' ')[0] || 'Admin'}</span>
            </Link>
          </div>
        </header>
        ...
        <div className="fixed inset-0 z-50 lg:hidden flex">
          ...
          <div className="relative w-64 bg-white dark:bg-slate-900 h-full flex flex-col z-10 p-4 border-r border-slate-200 dark:border-slate-800">
            <div className="flex items-center justify-between pb-4 border-b border-slate-200 dark:border-slate-800">
              <div className="font-extrabold text-blue-600">SADBHAWANA ADMIN</div>
