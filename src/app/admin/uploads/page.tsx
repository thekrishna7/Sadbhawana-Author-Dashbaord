...
function AdminUploadsContent() {
  ...
  const [fileType, setFileType] = useState('COVER_DESIGN');
  const [description, setDescription] = useState('');
  const [uploading, setUploading] = useState(false);

  // Tabs for Selected Book Files
  const [fileTab, setFileTab] = useState<'RECEIVED' | 'SENT' | 'ALL'>('RECEIVED');
  ...
  const handleUploadSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showToast('File uploaded successfully! Author notified.', 'success');
      setFile(null);
      setDescription('');
  ...
  const receivedFiles = bookFiles.filter((f) => f.uploader?.role === 'AUTHOR');
  const sentFiles = bookFiles.filter((f) => f.uploader?.role === 'ADMIN');
  const currentFilesList =
    fileTab === 'RECEIVED' ? receivedFiles : fileTab === 'SENT' ? sentFiles : bookFiles;
      ...
      {!selectedBook ? (
        <div className="space-y-6">
          <div>
            <h1 className="text-2xl font-extrabold text-slate-900 dark:text-slate-100 flex items-center gap-2.5">
              <FolderUp className="w-7 h-7 text-blue-600" /> Book Selection & Upload Center
            </h1>
            <p className="text-sm text-slate-500">
              Select a book title below to upload new files, inspect received manuscripts from authors, or review send/receive document history.
            </p>
          </div>

          {fetchingBooks ? (
            <div className="text-center py-16 text-slate-400">Loading catalog books...</div>
          ) : books.length === 0 ? (
            <div className="text-center py-16 bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 p-8 shadow-sm">
              ...
              <h3 className="text-lg font-bold text-slate-800 dark:text-slate-200">No Books Found in Catalog</h3>
              <p className="text-xs text-slate-500 max-w-sm mx-auto mt-1">
                Create a new book first in the Books management tab to start uploading files.
              </p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {books.map((book) => {
                const authorsList = book.assignments?.map((a: any) => a.author?.fullName).join(', ');
                return (
                  <div
                    key={book.id}
                    ...
                    <div>
                      <div className="flex items-start gap-4 mb-4">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {authorsList && (
                            <p className="text-[11px] text-indigo-600 dark:text-indigo-400 font-semibold mt-1 truncate flex items-center gap-1">
                              <User className="w-3 h-3" /> Author: {authorsList}
                            </p>
                          )}
                        </div>
                      </div>
                      ...
                      <p className="text-xs text-slate-600 dark:text-slate-400 line-clamp-2 mb-4">
                        {book.description || 'Click to select this title for uploading files or reviewing author submissions.'}
                      </p>
                    </div>

                    <div className="pt-3 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between">
                      <span className="text-xs text-slate-500 font-medium">Files: <strong className="text-blue-600">{book.files?.length || 0}</strong></span>
                      <button
                        onClick={() => selectBook(book)}
        ...
        <div className="space-y-8">
          ...
          <div className="space-y-4">
            ...
            >
              <ArrowLeft className="w-4 h-4" /> Back to All Books Catalog
            </button>

            <div className="p-6 rounded-3xl bg-gradient-to-r from-slate-900 via-blue-950 to-slate-900 text-white shadow-xl flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
          ...
          <div className="p-6 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm space-y-4">
            <div className="flex items-center justify-between pb-3 border-b border-slate-100 dark:border-slate-800">
              <h3 className="font-bold text-slate-900 dark:text-slate-100 text-base flex items-center gap-2">
                <Upload className="w-5 h-5 text-blue-600" /> Upload Admin File for "{selectedBook.name}"
              </h3>
            ...
            <form onSubmit={handleUploadSubmit} className="space-y-4">
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">
                  Upload Notes / Instructions for Author
                </label>
                <textarea
                  rows={3}
                  placeholder="e.g. Approved cover design attached. Please review layout..."
                  value={description}
              ...
              <div className="flex justify-end">
                ...
                >
                  <Upload className="w-4 h-4" /> {uploading ? 'Uploading...' : 'Send File to Author'}
                </button>
              </div>
            </form>
          </div>
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-2 border-b border-slate-200 dark:border-slate-800 pb-3">
              ...
              >
                <Inbox className="w-4 h-4 text-emerald-400" /> Received Files from Author ({receivedFiles.length})
              </button>
              ...
              >
                <Send className="w-4 h-4 text-indigo-400" /> Sent Files by Admin ({sentFiles.length})
              </button>
              ...
              >
                <FolderUp className="w-4 h-4 text-amber-400" /> All Book Files ({bookFiles.length})
              </button>
            </div>
            ...
            {fetchingFiles ? (
              ...
              <div className="text-center py-12 bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 p-6">
                ...
                <h4 className="font-bold text-slate-700 dark:text-slate-300">No Files in this Category</h4>
                <p className="text-xs text-slate-500 mt-1">No files match the selected filter tab.</p>
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  ...
                  >
                    <div>
                      ...
                      <p className="text-xs text-slate-500 mb-1">
                        Uploader: <strong className="text-slate-700 dark:text-slate-300">{file.uploader?.fullName}</strong> ({file.uploader?.role})
                      </p>
                    ...
                    <div className="pt-3 border-t border-slate-100 dark:border-slate-800 flex flex-col gap-2">
                      ...
                      {file.uploader?.role === 'AUTHOR' && file.status !== 'APPROVED' && (
